...
runners.forEach(function(person) {
  //get first name and store inside variable
  const first = (person.first_name);
  //get last name and store in variable
...
const firstNamesAllCaps = runners.map(function(currentPerson){
  return currentPerson.first_name.toUpperCase();
})
...
let runnersLargeSizeShirt = runners.filter(currentPerson => currentPerson.shirt_size === "L" );
...
let ticketPriceTotal = runners.reduce(function(accumulater, currentPerson){
  return accumulater + currentPerson.donation
}, 0);
